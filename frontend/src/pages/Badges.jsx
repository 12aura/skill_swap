...
// ── Level config (must match xpUtils.js) ──────────────────
const LEVELS = [
...
// ── All possible badges (must match xpUtils.js) ──────────
const ALL_BADGES = [
...
const Badges = () => {
  ...
  // Fetch fresh user data (with xp + badges)
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setProfile(user); // fallback to context
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className={`min-h-screen ${dm ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-900"}`}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
            ...
            <div className="flex-1 w-full">
              ...
              {nextLevel ? (
                ...
                <p className="text-sm text-yellow-400 font-semibold mt-2">
                  🏆 Max level reached — You're an Expert!
                </p>
              )}
        ...
        <div className={`rounded-3xl p-6 ${dm ? "bg-slate-800" : "bg-white"} shadow-md`}>
          <h3 className={`font-semibold text-lg mb-4 ${dm ? "text-white" : "text-slate-900"}`}>
            ⚡ How to Earn XP
          </h3>
            ...
            {[
              ...
              >
                ...
                <div className="flex-1">
                  <p className={`text-sm font-medium ${dm ? "text-white" : "text-slate-800"}`}>
                    {item.action}
                  </p>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className={`text-xl font-bold ${dm ? "text-white" : "text-slate-900"}`}>
              Badges
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {ALL_BADGES.map((badge) => {
              ...
              return earned ? (
                ...
                <div
                  key={badge.id}
                  className={`rounded-3xl p-6 shadow-md border border-teal-500/20 ${dm ? "bg-slate-800" : "bg-white"}`}
                >
                  ...
                  {badge.xpBonus > 0 && (
                    <p className="text-center text-yellow-400 text-xs font-semibold mt-2">
                ...
                <div
                  key={badge.id}
                  className={`rounded-3xl p-6 border-2 border-dashed ${dm ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}
                >
                  <div className="flex justify-center mb-4">
