...
async function checkAndAwardBadges(userId) {
  try {
    ...
    for (const def of BADGE_DEFINITIONS) {
      ...
      // ✅ Emit socket event instantly to the user's room
      // BadgeCelebration.jsx on the frontend listens for "badge-earned"
      if (global.io) {
        global.io.to(userId.toString()).emit("badge-earned", badgeData);
        console.log(`📡 Emitted badge-earned to socket room: ${userId}`);
      } else {
        console.warn("⚠️  global.io not set — badge socket event not sent");
      }
