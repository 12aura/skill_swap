...
// ─── Recording Banner ─────────────────────────────────────
// Shows a red dot banner to ALL participants when recording is active
function RecordingBanner({ isRecording }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 px-4 w-full justify-center bg-red-500/10 border-b border-red-500/20">
      ...
      <span className="text-red-400 text-xs font-medium">This session is being recorded</span>
    </div>
  );
}
...
// ─── Custom Controls ──────────────────────────────────────
function CustomControls({ onLeave, chatOpen, onToggleChat, onSendEmoji, unreadCount, isRecording }) {
  const call = useCall();
  const {
    useMicrophoneState,
    useCameraState,
...
// ─── VideoCallInner ───────────────────────────────────────
function VideoCallInner({ onLeave, onEndCall, userName, roomId, isRecording }) {
...
function VideoCall() {
  ...
  // ✅ All useState hooks are INSIDE the component — never outside
  ...
  // ✅ useEffect waits for consent modal to be dismissed before running
  useEffect(() => {
    if (!user || showConsentModal) return;
    hasLeftRef.current = false;
    initVideo(recordingConsent);
  }, [user, showConsentModal]);

  const initVideo = async (shouldRecord) => {
    try {
      ...
      // ✅ Only record if user gave consent
      if (shouldRecord) {
        try {
  ...
  // ✅ Cleanup on unmount — if user closes tab without clicking leave
  useEffect(() => {
    return () => {
  ...
  const leaveStream = async () => {
    try {
      if (isRecording) await callRef.current?.stopRecording();
  ...
  // ✅ Step 1 — Show consent modal BEFORE joining the call
  if (showConsentModal) {
    return (
      <div className="h-screen flex items-center justify-center bg-gray-950">
        <div className="bg-gray-900 border border-white/10 rounded-2xl p-8 max-w-md w-full mx-4">
          ...
          <h2 className="text-white text-xl font-semibold mb-2">Session Recording</h2>
          <p className="text-white/50 text-sm leading-relaxed mb-2">
            Would you like this session to be recorded? The recording will be available
            for both participants to rewatch after the call ends.
          </p>
          <p className="text-white/30 text-xs leading-relaxed mb-6">
            The other participant will see a banner notifying them that the session is being recorded.
          </p>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => {
                setRecordingConsent(true);
                setShowConsentModal(false); // ← triggers useEffect → initVideo(true)
              }}
              className="w-full py-3 bg-teal-500 hover:bg-teal-400 text-white rounded-xl font-medium transition"
            >
              Yes, record this session
            </button>
            <button
              onClick={() => {
                setRecordingConsent(false);
                setShowConsentModal(false); // ← triggers useEffect → initVideo(false)
              }}
              className="w-full py-3 bg-white/5 hover:bg-white/10 text-white/70 rounded-xl font-medium transition"
            >
              No thanks, join without recording
            </button>
          </div>
          <p className="text-white/20 text-xs text-center mt-4">
            Recordings are stored for 30 days and only visible to session participants.
          </p>
        </div>
      </div>
    );
  ...
  // ✅ Step 2 — Show loading spinner while call initialises
  if (!client || !call) {
    return (
